...
TEST_CASE("test-division", "[highs_benders]") {
  ...
  */
  ...
  REQUIRE(result.master_only_rows == std::set<HighsInt>{0});
}
...
TEST_CASE("test-division-v3", "[highs_benders]") {
    ...
    */
    ...
    REQUIRE(result.master_only_rows == std::set<HighsInt>{});
}
...
TEST_CASE("test-division-full-master", "[highs_benders]") {
    ...
    */
    ...
    REQUIRE(result.mixed_rows == std::set<HighsInt>{});
    REQUIRE(result.master_only_rows == std::set<HighsInt>{0, 1, 2, 3, 4, 5});
}
...
TEST_CASE("test-create-master", "[highs-benders]") {
  ...
  */
  ...
  RowDivision rd;
  rd.master_rows = {0, 3, 4};
  rd.master_only_rows = {0};
  rd.subproblem_rows = {1, 3, 4};
  rd.mixed_rows = {3, 4};
  Highs master;
  create_master_problem(master, lp, master_variables, rd.subproblem_rows, -inf, 1);
  ...
  REQUIRE(result.equalButForNames(expected));
}
...
TEST_CASE("test-create-subproblem", "[highs-benders]") {
  ...
  std::set<HighsInt> master_variables {0, 1};
  HighsLp expected;
  std::vector<HighsInt> csr_index {2,0,2,0,1,2};
  std::vector<double> csr_values(6, 1);
  std::vector<HighsInt> csr_starts {0,1,1,3,6};
  expected.offset_ = 0;
  expected.num_col_ = 3;
  expected.num_row_ = 4;
  expected.col_lower_ = {0, 0, 0};
  expected.col_upper_ = {inf, inf, inf};
  expected.col_cost_ = {0, 0, -1};
  expected.row_lower_ = {-inf,0,-inf, 1.5};
  expected.row_upper_ = {1,0, 2, inf};
  ...
  expected.a_matrix_.num_row_ = 4;
  expected.a_matrix_.num_col_ = 3;

  Highs subproblem;
  auto div = divide_rows(lp.a_matrix_.index_, lp.a_matrix_.start_, master_variables);
  create_subproblem(subproblem, lp, master_variables, div.master_only_rows);
...
TEST_CASE("test-create-feas-subproblem", "[highs-benders]") {
  ...
  */
  ...
  std::vector<HighsInt> csr_index {2, 0, 2, 3, 0, 1, 2, 4};
  std::vector<double> csr_values {1, 1, 1, -1, 1, 1, 1, 1};
  std::vector<HighsInt> csr_starts {0, 1, 1, 4, 8};
  HighsLp expected;
  expected.offset_ = 0;
  expected.num_col_ = 5;
  expected.num_row_ = 4;
  ...
  create_feasibility_subproblem(subproblem, lp, master_variables, division.mixed_rows, division.master_only_rows);
  auto result = subproblem.getLp();
  result.ensureRowwise();
...
TEST_CASE("test-get-multipliers", "[highs-benders]") {
  ...
  Highs subproblem;
  auto div = divide_rows(lp.a_matrix_, master_variables);
  create_subproblem(subproblem, lp, master_variables, div.master_only_rows);
...
TEST_CASE("test-add-objective-cut", "[highs-benders]") {
  ...
  add_nonzero_row(master, cut.rhs, kHighsInf, cut.coefficients);
