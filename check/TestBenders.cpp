...
void run(std::string corefile, std::string timefile, std::string stochfile, std::string note,
  int no_master_vars, int no_sub_vars, int no_master_rows, int no_sub_rows, int no_subs, double subproblem_lb) {
  Highs highs;
  REQUIRE(build_stochastic_problem(highs, corefile, timefile, stochfile));
...
void pure_run(std::string corefile, std::string timefile, std::string stochfile, std::string note,
  double subproblem_lb) {
  ...
  auto core_and_tree = build_stochastic_tree(corefile, timefile, stochfile);
  auto res = benders_l_shaped(core_and_tree.first, core_and_tree.second, starting_point, subproblem_lb, 1e-3, 100, true);
  res.note(0, note);
  REQUIRE(res == BendersRet {0, -1});
}

// TEST_CASE("test-benders-solve-multi-block-max-bigger-smps", "[highs-benders]") {
//   auto instance = std::string(HIGHS_DIR) + "/check/instances/stoch/pltexp/pltexpA";
//   pure_run(instance + "6.cor", instance + "6.tim", instance + "6_6.sto", "multi-block-big", -100);
// }


// TEST_CASE("test-scenario-weird", "[highs-benders]") {
//   auto instance = std::string(HIGHS_DIR) + "/check/instances/stoch/pltexp/pltexp";
//   auto corefile = instance + "A5.cor";
//   auto timefile = instance + "A5.tim";
//   auto stochfile = instance + "B5_6.sto";
//   Highs highs;
//   auto coresize = core_size(corefile);
//   REQUIRE(build_stochastic_problem(highs, corefile, timefile, stochfile));
//   highs.run();
//   REQUIRE(highs.getNumCol() == 188 + (coresize.first-188) *48);
//   REQUIRE(highs.getNumRow() == 62 + (coresize.second-62) * 48);
//   REQUIRE(highs.getObjectiveValue() == 0);
